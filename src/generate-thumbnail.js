...
async function generateThumbnail(filePath) {
    ...
    if (ext === '.pdf') {
        try {
            const pngBuffers = await pdfToPng(filePath, {
            ...
            if (pngBuffers.length > 0) {
                const buffer = pngBuffers[0].content;

                await sharp(buffer)
                    .resize(300, 300, { fit: 'contain', background: { r: 255, g: 255, b: 255} }) // Redimensiona para o tamanho do thumbnail
                    .jpeg({ quality: 70 })
                    .toFile(outputThumbnail);

                console.log(`Thumbnail gerada para o PDF: ${fileName}`);
